...
class CountriesDashboardApp extends React.Component {
    ...
    displayCountries = () => {
        ...
        let countries = filteredCountriesList.map(eachObj =>
            <div key={eachObj.alpha3Code} onClick={()=>this.countryDetails(eachObj)} className="shadow-xl my-2 " id={eachObj.name}>
                <img src={eachObj.flag} alt="flag" style={{width:"210px"}}  id={eachObj.name}/>
    ...
    render() {
        return <div className={this.props.selectedTheme==="Light mode"?"bg-white m-5":"bg-gray-800 text-white m-5"}> 
            <Header selectedTheme={this.props.selectedTheme} onChangeTheme={this.props.onChangeTheme}/>
            <div>
                <CountriesFilterBar onChangeSearchText={this.onChangeSearchText} onChangeSelectedRegion={this.onChangeSelectedRegion} selectedTheme={this.props.selectedTheme}/>
